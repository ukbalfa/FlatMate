...
- **Server Actions** in `app/actions/` use Firebase Admin SDK for privileged operations (create/delete data).
- **API routes** under `app/api/` — currently only `api/auth/` for session management.
...
| `app/dashboard/` | Protected area: expenses, balances, cleaning, tasks, roommates, settings, rates, announcements, flat management |
...
- **Firestore**: Real-time via `onSnapshot`. Multi-tenant isolation enforced by Firestore Security Rules (deploy separately).
...
- Copy `.env.example` → `.env.local` before running. Requires Firebase client + admin credentials, hCaptcha keys, and `SESSION_SECRET`.
